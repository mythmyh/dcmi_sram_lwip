...
//static  uint8_t testsram[20000] __attribute__ ((aligned(4)));

//static  uint32_t testsram[10000]
...
//
//	 return 0;
	data_len=(int)(160000)-__HAL_DMA_GET_COUNTER(&hdma_dcmi);
	 printf("%d\n",data_len*4);//得到剩余数据长度
...
static FRESULT ETX_MSC_ProcessUsbDevice2(char * filename,uint8_t  data[])
{
  ...
  do
  {
	...
	if((data[i]==0xff) &&(data[i+1]==0xd9)){
		...
		f_write(&file,data+(i+1), 1,(void *)&byteswritten);
		break;
...
{
//	__HAL_DCMI_CLEAR_FLAG(&hdcmi,DCMI_FLAG_FRAMERI);//清除帧中�?
//
//	printf("hello world 5\n");
//	__HAL_DMA_DISABLE(&hdma_dcmi);
//	while(DMA2_Stream1->CR&0X01);
//	data_len=5000-__HAL_DMA_GET_COUNTER(&hdma_dcmi);
...
//
//
//	ETX_MSC_ProcessUsbDevice2("b.jpg",testsram);
//
